...
function dashPath(r: Role) {
  return r === "Reader" ? "/dashboard/reader" : r === "Subject Expert" ? "/dashboard/subject-expert" : r === "Client" ? "/dashboard/business" : "/dashboard/writer";
}
...
function Inner() {
  ...
  const handleRole = (r: Role) => {
    setRole(r);
    window.history.replaceState(null, "", `/login?role=${encodeURIComponent(r)}`);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try { localStorage.setItem("sd_role", role); localStorage.setItem("sd_email", email); } catch (_) {}
    setLoading(false);
    ...
    <div className="flex min-h-screen">
      ...
      <main className="w-full md:ml-[40%] md:w-[60%] min-h-screen bg-[#FAFBFC] overflow-y-auto">
        ...
        <nav className="px-10 mb-4">
          <p className="text-sm text-gray-400">Login&gt;<span className="text-gray-600 font-medium">{role}</span></p>
        </nav>
        <div className="flex justify-center px-6 pb-16">
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 w-full max-w-md px-10 py-10">
            ...
            <div className="flex gap-3 mb-6">
              {[
                { l:"Apple", i:<svg viewBox="0 0 24 24" width="20" height="20" fill="currentColor"><path d="M17.05 20.28c-.98.95-2.05.8-3.08.35-1.09-.46-2.09-.48-3.24 0-1.44.62-2.2.44-3.06-.35C2.79 15.25 3.51 7.7 9.05 7.4c1.27.07 2.15.75 2.88.8.93-.1 1.81-.82 3.02-.88 2.67-.14 4.66 1.8 4.74 4.64-3.05 1.15-3.53 4.9-.64 6.32zm-3.4-13.14c-.2 1.97-2.07 3.64-4.07 3.52-.2-1.91 1.72-3.7 4.07-3.52z"/></svg> },
                { l:"Google", i:<svg viewBox="0 0 24 24" width="20" height="20"><path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/><path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/><path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/><path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/></svg> },
                { l:"Facebook", i:<svg viewBox="0 0 24 24" width="20" height="20" fill="#1877F2"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg> },
              ].map(s => (
                <button key={s.l} type="button" aria-label={`Sign in with ${s.l}`}
                  className="flex-1 flex items-center justify-center border border-gray-200 rounded-2xl py-3 hover:bg-gray-50 transition-colors cursor-pointer">{s.i}</button>
              ))}
            </div>
            ...
            <p className="text-sm text-gray-500 mb-6">Don&apos;t have an account? <Link href={`/signup?role=${encodeURIComponent(role)}`} className="text-blue-500 hover:underline">Sign up</Link></p>
            <form onSubmit={handleSubmit} noValidate className="flex flex-col gap-6">
              ...
              <div>
                ...
                <input type="password" value={password} onChange={e => setPassword(e.target.value)} className={inp} placeholder="Your password"/>
              </div>

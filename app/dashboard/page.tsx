...
function DonutChart() {
  ...
  const completeDash = (complete / total) * circumference;
  const inprogressDash = (inprogress / total) * circumference;
  const pendingDash = circumference - completeDash - inprogressDash;
  return (
    <svg width="140" height="140" viewBox="0 0 140 140">
      ...
      <circle cx={cx} cy={cy} r={r} fill="none" stroke="#e5e5e5" strokeWidth="16"
        strokeDasharray={`${completeDash} ${circumference - completeDash}`}
      ...
      <circle cx={cx} cy={cy} r={r} fill="none" stroke="#888" strokeWidth="16"
        ...
        strokeDashoffset={circumference * 0.25 - completeDash}
        strokeLinecap="round"
      />
      <text x={cx} y={cy - 6} textAnchor="middle" fill="white" fontSize="10" fontWeight="400">Complete Task</text>
...
function MiniBarChart() {
  ...
  return (
    <svg width="100" height="50" viewBox="0 0 100 50">
      {bars.map((h, i) => (
        <rect key={i} x={i * 14 + 2} y={50 - (h / 100) * maxH - 8} width="10" height={(h / 100) * maxH + 4} rx="2" fill={i === 6 ? "white" : "#555"} />
      ))}
    </svg>
  );
}
...
function TrendsChart() {
  return (
    <svg width="100%" height="120" viewBox="0 0 300 120" preserveAspectRatio="none">
      ...
      <path d="M0,80 C30,70 60,30 90,50 C120,70 150,20 180,40 C210,60 240,30 270,20 L300,15 L300,120 L0,120 Z" fill="url(#trendGrad)" />
      <path d="M0,80 C30,70 60,30 90,50 C120,70 150,20 180,40 C210,60 240,30 270,20 L300,15" stroke="white" strokeWidth="2" fill="none" />
      <circle cx="240" cy="30" r="4" fill="white" />
      <rect x="230" y="10" width="50" height="20" rx="4" fill="#333" />
      <text x="255" y="23" textAnchor="middle" fill="white" fontSize="8">Rs.XXX</text>
    </svg>
  );
}
...
function PieChart() {
  return (
    <svg width="80" height="80" viewBox="0 0 80 80">
      ...
      <circle cx="40" cy="40" r="30" fill="none" stroke="#888" strokeWidth="20"
        strokeDasharray="60 188" strokeDashoffset="47" />
      <circle cx="40" cy="40" r="30" fill="none" stroke="#aaa" strokeWidth="20"
        strokeDasharray="40 188" strokeDashoffset="-13" />
      <text x="40" y="36" textAnchor="middle" fill="white" fontSize="7">Total</text>
      <text x="40" y="47" textAnchor="middle" fill="white" fontSize="7">Audience</text>
      <text x="40" y="57" textAnchor="middle" fill="white" fontSize="9" fontWeight="700">xyz</text>
    </svg>
  );
}
...
function PerfChart() {
  return (
    <svg width="100%" height="130" viewBox="0 0 340 130" preserveAspectRatio="none">
      {[100, 80, 60, 40, 20, 0].map((y, i) => (
        <g key={i}>
          <text x="0" y={10 + i * 22} fill="#aaa" fontSize="9">{100 - i * 20}</text>
          <line x1="24" y1={10 + i * 22} x2="340" y2={10 + i * 22} stroke="#e5e5e5" strokeWidth="0.5" />
        </g>
      ))}
    ...
    <div className="min-h-screen bg-[#f4f4f4] p-6">
      ...
      <div className="flex items-start justify-between mb-6">
        ...
        <div className="flex items-center gap-2">
          ...
          <button className="bg-white text-gray-600 text-xs font-medium px-4 py-2 rounded-full border border-gray-200 hover:bg-gray-50 transition-colors">Select Date</button>
          <button className="bg-white text-gray-600 text-xs font-medium px-4 py-2 rounded-full border border-gray-200 flex items-center gap-1 hover:bg-gray-50 transition-colors">
      ...
      <div className="bg-black rounded-2xl p-5 mb-4 flex items-center gap-6">
        ...
        <div className="ml-auto bg-[#2a2a2a] rounded-xl p-3 flex items-center gap-3 w-64">
          ...
          <div className="flex-1">
            ...
            <p className="text-gray-400 text-[10px] leading-relaxed line-clamp-2">Lorem ipsum dolor sit amet, consectetur adipiscing elit.</p>
      ...
      <div className="grid grid-cols-2 gap-4 mb-4">
        ...
        <div className="bg-white rounded-2xl p-5">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              <div className="flex gap-0.5">
                <div className="w-2.5 h-5 bg-black rounded-sm" />
                <div className="w-2.5 h-4 bg-gray-400 rounded-sm" />
                <div className="w-2.5 h-3 bg-gray-300 rounded-sm" />
              </div>
              <span className="text-sm font-semibold text-gray-800">Your Top Articles</span>
            </div>
            <button className="text-xs text-gray-400 hover:text-gray-700">Details</button>
          </div>
          {[1,2,3].map(n => (
        ...
        <div className="bg-black rounded-2xl p-5">
          <div className="flex items-center justify-between mb-2">
            ...
            <div className="flex items-center gap-1 bg-[#2a2a2a] text-gray-300 text-xs px-3 py-1 rounded-full">
              <span>Payment</span>
              <ChevronDown size={11} />
            </div>
          </div>
          <div className="text-gray-500 text-xs flex justify-end mb-1">
            {["Mon","Tue","Wed","Thurs","Fri","Sat","Sun"].map(d => <span key={d} className="flex-1 text-center">{d}</span>)}
          </div>
          <TrendsChart />
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-4 mb-4">
        ...
        <div className="bg-white rounded-2xl p-5">
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="flex items-center gap-1 border border-gray-200 text-gray-500 text-xs px-2 py-1 rounded-full">
              <span>This Week</span>
              <ChevronDown size={10} />
            </div>
          </div>
          ...
          <div className="flex items-end gap-1">
            <MiniBarChart />
            <span className="text-sm font-bold text-gray-800 ml-1">+15</span>
          </div>
          <div className="flex justify-between text-xs text-gray-400 mt-1">
            <span>TOTAL</span><span>1.2k</span>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl p-5">
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="border border-gray-200 text-gray-500 text-xs px-2 py-1 rounded-full">This Week</div>
          </div>
          {[
            { icon: "⊙", name: "Category Name", pop: "10.1K" },
            { icon: "⊙", name: "Category Name", pop: "10.1K" },
            { icon: "⊙", name: "Category Name", pop: "10.1K" },
          ].map((cat, i) => (
            <div key={i} className="flex items-center gap-3 py-2.5 border-b border-gray-100 last:border-0">
              <div className="w-8 h-8 rounded-full bg-gray-100 flex items-center justify-center text-sm">{cat.icon}</div>
              <div>
                <p className="text-sm font-medium text-gray-800">{cat.name}</p>
                <p className="text-xs text-gray-400">Popularity {cat.pop}</p>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="bg-black rounded-2xl p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <button className="text-gray-400 text-xs hover:text-white">Details</button>
          </div>
          ...
          <div className="flex gap-2 mt-3">
            ...
            <div className="h-6 flex-1 bg-gray-600 rounded" />
          </div>
        </div>
      </div>
        ...
        <div className="grid grid-cols-3 gap-4">
          <div className="col-span-2 bg-black rounded-2xl p-5">
            ...
            <div className="flex divide-x divide-gray-700">
              {[
                { label: "impressions", val: "12k" },
                { label: "Likes", val: "12k" },
                { label: "Feedbacks", val: "12k" },
              ].map((item) => (
          ...
          <div className="bg-black rounded-2xl p-5 flex flex-col items-center justify-center">
            ...
            <p className="text-xs text-gray-400 mb-3">Lorem ipsum dolor</p>

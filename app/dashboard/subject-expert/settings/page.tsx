          ...
          <div style={{minWidth:260}}>
            <div className="bg-white rounded-2xl p-5 border border-gray-100">
              ...
              <div className="flex flex-col gap-4">
                {[["Phone","+628102512288","tel"],["Email","richardtyson@gmail.com","email"],["Address","Merdeka Street, Wonosobo","text"],["Password","","password"],["Bank","","text"],["IFSC code","","text"]].map(([l,v,t]) => (
                  <div key={l as string}>
                ...
                <div>
                  ...
                  <textarea defaultValue="An active Tokopedia data kelebihan application myself to my goal, I want to help." rows={3} className="w-full border-b border-gray-200 bg-transparent outline-none focus:border-gray-500 py-1.5 text-xs text-gray-600 resize-none"/>
                </div>
              </div>
          ...
          <div className="flex-1">
            <div className="bg-white rounded-2xl p-5">
              <div className="flex items-center justify-between mb-4">
                <p className="text-gray-900 text-sm font-semibold">My Clients</p>
                    ...
                    <tr key={c.id} className="border-t border-gray-50">
                      <td className="py-2.5">
                        <div className="flex items-center gap-2">
                          <div className="w-7 h-7 rounded-full bg-gradient-to-br from-orange-300 to-orange-500 flex items-center justify-center text-white text-xs font-bold">{c.name[0]}</div>
                          <div><p className="text-sm text-gray-800 font-medium">{c.name}</p><p className="text-xs text-gray-400">{c.role}</p></div>
                        </div>
                      </td>
                      <td className="py-2.5 text-xs text-gray-500">{c.email}</td>
                      <td className="py-2.5 text-xs text-gray-500">{c.phone}</td>
                      <td className="py-2.5 text-xs text-gray-500">{c.id}</td>
                    </tr>
                  ))}
                </tbody>
              </table>

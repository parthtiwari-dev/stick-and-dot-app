...
function MeshCard({ card, style, size }: { card: typeof CARDS[0]; style: typeof CARD_STYLES[0]; size: "sm"|"lg" }) {
  const h = size === "lg" ? 400 : 300;
  return (
    <div className="relative rounded-3xl overflow-hidden select-none w-full" style={{ height: h, background: style.bg }}>
      <div className="absolute inset-0 overflow-hidden">
        <div className="absolute rounded-full opacity-45 blur-3xl" style={{ width:240, height:240, background:style.blob1, top:-80, left:-60 }} />
        <div className="absolute rounded-full opacity-30 blur-3xl" style={{ width:200, height:200, background:style.blob2, bottom:0, right:-50 }} />
        <div className="absolute rounded-full opacity-20 blur-2xl" style={{ width:160, height:160, background:style.blob3, top:"50%", left:"50%" }} />
      </div>
      <div className="absolute top-4 left-4 flex items-center gap-2">
        <span className="bg-white/10 backdrop-blur-md border border-white/20 text-white/70 text-xs px-3 py-1 rounded-full">{card.tag}</span>
      </div>
      {size === "lg" && (
      ...
      <div className="absolute bottom-0 left-0 right-0 p-5 bg-gradient-to-t from-black/60 to-transparent pt-12">
        <h3 className={`text-white font-bold leading-snug mb-2 ${size === "lg" ? "text-lg" : "text-sm"}`}>{card.title}</h3>
        <div className="flex items-center gap-2">
          <div className="w-6 h-6 rounded-full bg-gradient-to-br from-orange-400 to-pink-500 flex-shrink-0 flex items-center justify-center text-white text-xs font-bold">{card.author[0]}</div>
        ...
        <div className="flex-1 flex flex-col items-center">
          <div className="relative w-full flex items-center justify-center">
            ...
            <div className="flex items-center gap-4 md:gap-5 justify-center">
              <div onClick={prev} className="hidden sm:block cursor-pointer flex-shrink-0 transition-all duration-500 opacity-35 hover:opacity-55 scale-90 hover:scale-[0.93]" style={{ width: 210 }}>
              ...
              <div className="flex-shrink-0 transition-all duration-500" style={{ width: "min(380px, 90vw)" }}>
                <Link href={`/articles/${current.id}`}>
                  <div className="cursor-pointer hover:scale-[1.02] transition-all duration-300" style={{ filter:"drop-shadow(0 0 50px rgba(255,255,255,0.06))" }}>
                    <MeshCard card={current} style={CARD_STYLES[active % CARD_STYLES.length]} size="lg" />
                  </div>
                </Link>
                <div className="mt-5 flex items-center justify-between px-1">
                  <div><p className="text-white/70 text-xs font-medium mb-0.5">Published</p><p className="text-white/35 text-xs">10 / 2 / 2023</p></div>
                  <Link href={`/articles/${current.id}`}>
                  ...
                  <div className="text-right"><p className="text-white/70 text-xs font-medium mb-0.5">Engagement</p><p className="text-white/35 text-xs">{current.views} views</p></div>
                </div>
              </div>
              <div onClick={next} className="hidden sm:block cursor-pointer flex-shrink-0 transition-all duration-500 opacity-35 hover:opacity-55 scale-90 hover:scale-[0.93]" style={{ width: 210 }}>
          ...
          <div className="flex items-center gap-2 mt-10">
            {CARDS.map((_, i) => (
              <button key={i} onClick={() => setActive(i)}
                className={`rounded-full transition-all duration-300 cursor-pointer ${i === active ? "w-7 h-2 bg-white" : "w-2 h-2 bg-white/20 hover:bg-white/40"}`} />
            ))}
          </div>
            ...
            <div className="flex flex-col gap-1">
              {CARDS.map((c, i) => (
                <button key={c.id} onClick={() => setActive(i)}
                  className={`flex items-center justify-between px-4 py-3 rounded-2xl cursor-pointer transition-all ${i === active ? "bg-white/10 border border-white/15" : "hover:bg-white/5 border border-transparent"}`}>
